"""Arquivo referente a classe de authentication do user"""
...
class Authentication: 
    def signin(self, email=None, password=None) -> User:
        """
            Metodo responsável pelo Sign-in do usuário
        """
    ...
    def signup(self, name, email, password, type_account='owner', company_id=False):
        ...
        if not password or password == '': 
            raise APIException('O password não pode ser null.')
        ...
        if type_account == 'employee':
            Employee.objects.create(
                enterprise=company_id or created_enterprise.id, 
